...
public Action:Timer_IsMatchStart(Handle:timer) {
		...
		for (new i = 1; i <= MaxClients; i++) {
			...
			if (IsClientConnected(i) && IsClientInGame(i) && !IsFakeClient(i)) {
				...
				if (i_IsHudDisabled == 0 && !b_IsHideHud[i]) SendPanelToClientAndClose(ReadyUpMenu(i), i, ReadyUpMenu_Init, 1);
					...
					if (g_ForceReadyUpStartTimer == INVALID_HANDLE) {
	...
	if ((!IsEligibleMap(0) && i_ReadyUpTime < 1 || IsEligibleMap(0) && i_ReadyUpTimeFirst < 1) || IsEligiblePlayersReady() && !IsClientsLoading() || IsEligiblePlayersReady() && b_IsAllClientsLoaded) {
