...
stock ClearAndLoad(String:key[])
{
	...
	Format(tquery, sizeof(tquery), "SELECT `steam_id`, `strength`, `luck`, `agility`, `technique`, `endurance`, `experience`, `experience overall`, `upgrade cost`, `level`, `%s`, `time played`, `talent points`, `total upgrades`, `free upgrades`, `slate points`, `handicap`, `rested time`, `rested experience`, `last play length`, `resr`, `pri`, `tcolour`, `tname`, `ccolour` FROM `%s` WHERE (`steam_id` = '%s');", GetConfigValue("sky points menu name?"), GetConfigValue("database prefix?"), key);
...
public QueryResults_Load(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	if ( hndl != INVALID_HANDLE )
	{
		...
		if (PlayerLevel[client] > 0) {
			...
			if (PlayerLevel[client] > PlayerTalentLevel(client) + 1) {

				PlayerLevel[client] = PlayerTalentLevel(client) + 1;
				HandicapLevel[client] = -1;		// see above.
			}
